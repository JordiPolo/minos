...
pub fn check_status(response_status: StatusCode, desired_status: StatusCode, location: &Location) -> Option<Disparity> {
...
pub fn check_response_type(value: &JsonValue, schema_type: &str, location: &Location) -> Option<Disparity> {
    let is_match = match value {
        ...
        JsonValue::Number(_) => {
            schema_type == "integer" || schema_type == "number"
        },
        JsonValue::Boolean(_) => {
            schema_type == "boolean"
        },
    ...
    if !is_match {
        let error = Disparity::new(
            &format!("Got {} from the service but described as {:?}", value, schema_type),
...
pub fn check_number_format(value: &JsonValue, _schema: &openapi::v2::Schema, _location: &Location,) -> Option<Disparity> {
    match value {
